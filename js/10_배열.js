...
function selectMenu(){
    ...
    // 점심 메뉴로 초기화된 배열 생성
    const menus = ["굶기", "김밥", "제육볶음","돈까스", "파스타", "샐러드","라면", "햄버거", "순대국밥", "닭갈비","인간","개","고양이"];

    // menus 배열 index 범위 내어서 난수 생성
...
/* 
  ...
  - 메뉴명 입력 시 취소를 클리갛면 "주문 완료"

  - 수량 입력 시 취소를 클리하면 해당 메뉴 주문만 취소
*/
function orderFn(){

    const tdody = document.getElementById("tbody");
    ...
    const counts = new Array(menus.length); // -> 모든 카 비어있음
