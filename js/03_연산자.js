...
function plusFn(){
    ...
    // [해결방법]
    // 문자열 (string)을 수자(number)로 변경한는 코드를 수행
...
/* 두 수의 합을 아이디가 "calcResult"인 요소(result 변수)의
    내부 굴자(innerText, HTML 요소 내용)으로 대입하기
*/
...
function check1(){
    ...
    // 빡에 작성환 변수 count, 안에 작성한 count는 서로 다른 변수(동명이인)
    let count = 100;

    // 컴퓨터한테 연산은 +-*/ 뿐만 아니라
    // 코드를 하나한 실해하는 것들이 연산이다!!
    ...
    console.log("후위 연산 후count:", count); // 50
